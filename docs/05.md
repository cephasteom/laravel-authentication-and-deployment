...
## Overview
There are many ways to deploy your Laravel API - many of them highly automated, efficient and pain free. Even though it's a slighly more involved process, we're going to use a preconfigured `Optimized LAMP Stack PHP 7 on Ubuntu` server from the Amazon Marketplace, because:
...
- Launch Instance
- Choose - Search for and choose `Optimized LAMP Stack PHP 7 on Ubuntu`
- Choose - T2.micro
...
- The instance should load up. Ensure you keep this browser window open.
- Go back to the EC2 dashboard. You should now see this instance.	 
...
For a Ubuntu instance the username is `ubuntu` and the domain can be found by clicking on your instance in the EC2 dashboard and copying the Public DNS (IPv4). It should look something look like this:
````
ssh ubuntu@ec2-3-8-39-24.eu-west-2.compute.amazonaws.com
````
...
````
ssh -i ~/.ssh/aws-key-pair.pem ubuntu@ec2-3-8-125-57.eu-west-2.compute.amazonaws.com
````
...
By default, on this Ubuntu server, your files are served from the directory `/jet/app/www/default/`. This is like your public folder in a vagrant box and is called the Document Root. This, and other useful information, can be found on this machine with the command:
````
usage
````

Checkout your server in the browser (in the EC2 dashboard - copy the Public DNS (IPv4)). You should see an `Index of /` page.
...
Better still, we've spun up a php server, so we should be able to do something more dynamic. Let's rename the index file to index.php, and open in a text editor:
````
...
````
<h1>Hello world. The time and date is currently <?php echo date('l jS \of F Y h:i:s A') ?></h1>
````
Sense Check! Checkout your server in the browser. This should now read Hello World and tell us the time and date. 
...
You should now be able to see this key pair in the ssh directory
````
cd ~/.ssh && ls
````
...
Go back to the page in the browser, you should still see the `Index of /` page - why is this? Your laravel project is actually served out of the public folder, whereas your server is setup to look for an index page at `/jet/app/www/default/`. So, we need to tell the server where the project root is.

Open the appropriate Apache config file in a text editor
````
nano /jet/etc/apache/conf.d/www.conf
````

Change DocumentRoot to
````
DocumentRoot "/jet/app/www/default/public"
````
And
````
<Directory "/jet/app/www/default/public">
````
...
You are missing your .env file with your laravel key in there. Create a new .env file and paste the contents of your existing .env into it. We'll need to change some of these variables but we'll come to that later...
````
nano /jet/app/www/default/.env
````
Exit and save.
...
````
nano /jet/app/www/default/app/Providers/AppServiceProvider.php
````
